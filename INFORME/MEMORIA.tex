...
\subsection{Motor Gráfico: Godot 4.x}
...
\begin{itemize}
    ...
    \item Generación de geometría de paredes
...
\subsubsection{Generación de Paredes}

Cada pared es un Node3D que contiene:
\begin{itemize}
    \item \textbf{Posición inicial}: (0, 2, 20)
    \item \textbf{Movimiento}: -5.0 unidades/segundo en eje Z
    \item \textbf{Geometry}: Malla generada con SurfaceTool
    \item \textbf{Material}: StandardMaterial3D azul transparente (0.2, 0.2, 0.8, 0.8)
    \item \textbf{Limpieza}: Se elimina cuando z < -30
\end{itemize}

\subsubsection{Geometría de Paredes}

Las paredes se generan usando SurfaceTool que:
\begin{itemize}
    \item Crea rectángulos sólidos en posiciones específicas
    \item Deja agujeros vacíos donde el personaje debe pasar
    \item Utiliza la función add\_rect() para construir la geometría
    \item Genera normales automáticamente con generate\_normals()
\end{itemize}
...
\begin{itemize}
    \item Posición Z de la pared está entre 1.0 y -1.0
    \item Sistema verifica si el jugador está dentro del área del agujero
    \item Si está dentro: +10 puntos
    \item Si está fuera: -1 vida
\end{itemize}
...
\begin{itemize}
    \item Se capturan rotaciones de MediaPipe
    \item Se interpolan las rotaciones con slerp() para suavidad
    \item Se actualiza cada frame según los datos de pose
    \item Los huesos se mapean a puntos de referencia de MediaPipe
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Posición inicial}: (0, 2, 20) - frente al jugador
    \item \textbf{Movimiento}: -5.0 unidades/segundo hacia el jugador
    \item \textbf{Limpieza}: Se elimina cuando sale del rango visible (z < -30)
\end{itemize}
...
\subsection{Generación de Geometría}
